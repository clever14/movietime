...
/* ══ STORAGE : localStorage en priorité, Supabase en arrière-plan ══ */
const LS = {
  get:(k)=>{ try{ const r=localStorage.getItem(k); return r?JSON.parse(r):null; }catch{ return null; } },
  set:(k,v)=>{ try{ localStorage.setItem(k,JSON.stringify(v)); }catch{} },
};

/* Supabase sync (non-bloquant) */
async function sbSync(path, opts){
  try{ await fetch(`${SB_URL}/rest/v1/${path}`,{headers:SB_H,...opts}); }catch{}
}
async function sbGet(path){
  try{ const r=await fetch(`${SB_URL}/rest/v1/${path}`,{headers:SB_H}); if(r.ok){ const t=await r.text(); return t?JSON.parse(t):null; } }catch{}
  return null;
}

/* ══ DATA LAYER ══ */
function loadRecords(){ return LS.get("mts-records")||[]; }
function saveRecords(recs){
  LS.set("mts-records",recs);
  // Sync to Supabase in background
  sbSync("records?id=neq.null",{method:"DELETE"}).then(()=>{
    if(recs.length>0) sbSync("records",{method:"POST",body:JSON.stringify(recs),headers:{...SB_H,"Prefer":"return=minimal"}});
  });
}
function loadCounter(){ return LS.get("mts-counter")||1; }
function saveCounterFn(n){
  LS.set("mts-counter",n);
  sbSync("app_counter?id=eq.1",{method:"PATCH",body:JSON.stringify({value:n}),headers:{...SB_H,"Prefer":"return=minimal"}});
}
function loadUsers(){
  return LS.get("mts-users")||[{id:"u1",login:"Yannick_Douo",name:"Yannick Douo",role:"admin",active:true}];
}
function saveUsersFn(users){
  LS.set("mts-users",users);
  sbSync("app_users?id=neq.null",{method:"DELETE"}).then(()=>{
    if(users.length>0) sbSync("app_users",{method:"POST",body:JSON.stringify(users),headers:{...SB_H,"Prefer":"return=minimal"}});
  });
}
function resetAllData(){
  LS.set("mts-records",[]);
  LS.set("mts-counter",1);
  sbSync("records?id=neq.null",{method:"DELETE"});
  sbSync("app_counter?id=eq.1",{method:"PATCH",body:JSON.stringify({value:1}),headers:{...SB_H,"Prefer":"return=minimal"}});
}

/* ══ CONSTANTS ══ */
const T={bg:"#080808",card:"rgba(255,255,255,0.04)",border:"rgba(255,255,255,0.08)",red:"#DC143C",gold:"#FFD700",green:"#10B981",blue:"#3B82F6"};
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    setUsers(loadUsers());
    // Poll localStorage every 3s for cross-tab sync
    const id=setInterval(()=>{ setRecords(loadRecords()); },3000);
    return()=>clearInterval(id);
  },[]);

  const updRecords=recs=>{ setRecords(recs); saveRecords(recs); };
  const updCounter=n=>{ setCounter(n); saveCounterFn(n); };
  const updUsers=u=>{ setUsers(u); saveUsersFn(u); };
  ...
  return(
    <>
      ...
      {page==="admin"  && <AdminApp  user={user} records={records} counter={counter} users={users} updRecords={updRecords} updCounter={updCounter} updUsers={updUsers} onLogout={logout}/>}
      {page==="server" && <ServerApp user={user} records={records} updRecords={updRecords} onLogout={logout}/>}
    </>
  );
}
...
/* ══════════ ADMIN APP ══════════ */
function AdminApp({user,records,counter,users,updRecords,updCounter,updUsers,onLogout}){
    ...
    <div style={{display:"flex",height:"100vh",background:"#080808",overflow:"hidden"}}>
      <aside style={{width:sideOpen?240:64,flexShrink:0,background:"rgba(255,255,255,0.025)",borderRight:"1px solid rgba(255,255,255,0.05)",display:"flex",flexDirection:"column",transition:"width 0.3s",overflow:"hidden"}}>
        ...
        <div style={{padding:"10px 8px",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          {sideOpen&&<div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:"9px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
            ...
            <div><div style={{fontSize:11,fontWeight:700,color:"#fff",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user?.name}</div><div style={{fontSize:9,color:T.gold}}>Administrateur</div></div>
          </div>}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:18}}>
          {tab==="pos"       && <POSView records={records} counter={counter} updRecords={updRecords} updCounter={updCounter}/>}
          {tab==="orders"    && <OrdersView records={records} updRecords={updRecords}/>}
          {tab==="stats"     && <StatsView records={records}/>}
          {tab==="inventory" && <InventoryView/>}
          {tab==="users"     && <UsersView users={users} updUsers={updUsers}/>}
          {tab==="settings"  && <SettingsView updRecords={updRecords} updCounter={updCounter}/>}
        </div>
      </div>
    </div>
  );
...
/* ══════════ POS ══════════ */
function POSView({records,counter,updRecords,updCounter}){
...
/* ══════════ ORDERS ══════════ */
function OrdersView({records,updRecords}){
  ...
  const handleServe=(code,cat)=>{
    const updated=records.map(r=>r.order_code===code&&getCategory(r.item_name)===cat?{...r,served:true}:r);
    updRecords(updated);
  };
  return(
    <div>
...
/* ══════════ USERS ══════════ */
function UsersView({users,updUsers}){
  ...
  const addUser=()=>{
    ...
    updUsers([...users,{id:`u${Date.now()}`,login:form.login,name:form.name,role:form.role,active:true}]);
    setModal(false); setForm({login:"",name:"",role:"server",password:""}); setErr("");
  };
  const toggle=id=>{ if(id==="u1")return; updUsers(users.map(u=>u.id===id?{...u,active:!u.active}:u)); };
...
/* ══════════ SETTINGS ══════════ */
function SettingsView({updRecords,updCounter}){
  const [modal,setModal]=useState(false);
...
/* ══════════ SERVER APP ══════════ */
function ServerApp({user,records,updRecords,onLogout}){
  ...
  const handleServe=(code,category)=>{
    const updated=records.map(r=>r.order_code===code&&getCategory(r.item_name)===category?{...r,served:true}:r);
    updRecords(updated);
  };
...
function ServerOrderCard({group,meta,onServe}){
  ...
  const handle=()=>{ setServing(true); onServe(); };
  return(
